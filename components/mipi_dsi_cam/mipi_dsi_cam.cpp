...
bool MipiDsiCam::start_streaming() {
  ...
  this->last_capture_sequence_ = this->total_frames_received_;
...
bool MipiDsiCam::acquire_frame(uint32_t &frame_sequence) {
  if (!this->streaming_) {
    return false;
  }

  // Tenter de récupérer un instantané cohérent des compteurs
  for (int attempt = 0; attempt < 3; attempt++) {
    uint32_t current_sequence = this->total_frames_received_;
    if (current_sequence == frame_sequence) {
      return false;
    }

    uint8_t buffer_index = this->buffer_index_;
    uint8_t last_buffer = (buffer_index + 1) % 2;

    uint32_t confirm_sequence = this->total_frames_received_;
    uint8_t confirm_index = this->buffer_index_;

    if (current_sequence != confirm_sequence || buffer_index != confirm_index) {
      continue;
    }
...
bool MipiDsiCam::capture_frame() {
  ...
  bool has_new_frame = this->acquire_frame(this->last_capture_sequence_);
  if (has_new_frame) {
...
void MipiDsiCam::loop() {
  if (this->streaming_) {
    ...
    uint32_t now = millis();
    uint32_t elapsed_ms = now - this->last_frame_log_time_;
    if (elapsed_ms >= 3000) {
      uint32_t frames_since_last_log = this->total_frames_received_ - this->last_fps_log_sequence_;
      float seconds = static_cast<float>(elapsed_ms) / 1000.0f;
      float sensor_fps = seconds > 0.0f ? frames_since_last_log / seconds : 0.0f;

      uint32_t total_checks = ready_count + not_ready_count;
      float ready_rate = total_checks > 0
                           ? (static_cast<float>(ready_count) / static_cast<float>(total_checks)) * 100.0f
                           : 0.0f;
      ...
      this->last_fps_log_sequence_ = this->total_frames_received_;
